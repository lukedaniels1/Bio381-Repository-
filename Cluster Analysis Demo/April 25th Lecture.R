...
iris.scaled <- scale(iris[,-5])  # Talk About Euclidean Distance How To 

# Step 1: Do Clusters Even Exist? 

fviz_pca_ind(prcomp(iris.scaled,   # Discuss Principle Component Analysis 
                    title = "Normal Data", 
                    palette = "jco", geom = "point"))

Hopkins <- get_clust_tendency(iris.scaled, # A numeric data frame or matrix
                              n=nrow(iris.scaled)-1, #The number of points selected from sample
...
distgower <- daisy(iris.scaled, # Matrix with either dissimilarity matrix or normal data
...
km.res <- kmeans(iris.scaled, centers = 6, 
...
aggregate(iris, by = list(cluster = km.res$cluster), mean)
swiss2 <- cbind(iris, cluster = km.res$cluster)
head(swiss2)

# Visualizing K- Means
fviz_cluster(km.res, # Object created by a clusting function such as kmeans
             data = iris, # The data used for clustering 
             choose.vars = c("Sepal.Length", "Petal.Width"), #We can cluster based on select variables 
...
fviz_nbclust(iris.scaled, # Numeric matrix or data frame
...
km.res <- eclust(iris.scaled, # A numeric data frame
                 stand = FALSE,
                 FUNcluster = "kmeans",  #The type of clustering you want to do 
                 hc_metric = "manhattan",
                 k = 6, #If NULL the Gap Stat is used to find the number of clusters 
                 nstart = 25, 
                 graph = TRUE)
...
km.res <- eclust(iris.scaled, 
                 "kmeans", 
                 k = 3,
                 stand = FALSE, #If TRUE then Eclust will standardize data
                 nstart = 25, 
                 graph = FALSE)
fviz_cluster(km.res, geom = "point", ellipse.type = "norm")
...
nb <- NbClust(iris.scaled, #Data Must Be Scaled 
              distance = "euclidean", 
              min.nc = 2, 
              max.nc = 10, 
              method = "ward.D2")
