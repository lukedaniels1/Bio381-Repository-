...
\ 
NbClust Package privdes 30 indeces for determining the number of clusters, distance measures, and clustering methods. 
...
We see condradicting results between the different methods. Some tests suggest 6 clusters while the NbClust suggests 2.  To move forward its important that we validate our results. 
...
We have also been using euclidean distances in our clusters.  If your data contains outliers Manhattan distances will give a more robust result. While Base R can calculate the PAM approach, the cluster package is much simpliler. 
...
2.) Determine Linkage - Take the distance information and group pairs into clusters until all obkects are linked together in a hierarchical tree
